...
def separate_landscape_and_points(matrix):
    ...
    return landscape, coord_points[1], coord_points[
        2]  # отдаем координаты x и y,причем парно(то есть первые две координаты принадлежат 0-ому ландшафтну)
...
def get_reject_and_result(nThread):  # для готовых данных
    ...
    for i in range(begin_el, end_el):
        if i % 1000 == 0:
            print(i)
        y_test, reject[i] = calculateResult(
            np.array([x_points[2 * i], y_points[2 * i], landscape[i, x_points[2 * i], y_points[2 * i]]]),
            np.array(
                [x_points[2 * i + 1], y_points[2 * i + 1], landscape[i, x_points[2 * i + 1], y_points[2 * i + 1]]]),
            landscape[i])
