...
def calculateWithResult():
    data3Points = np.zeros((count_landscapes, count_tuples, count_shifts,
                            count_points + 1, count_pixels, count_pixels),
                           dtype=np.float32)  # 1 - для самого ландшафта
    result = np.random.randint(2, size=(count_landscapes, count_tuples, count_shifts, count_points))  # 1-2;2-3;3-1
    rejects = np.zeros((count_landscapes, count_tuples, count_shifts, count_points),
                       dtype=np.float32)  # разница медлу прямой и ландшафтом
...
def calcMultiThreads(l):
    ...
    rejects = np.zeros((coef, count_tuples, count_shifts, count_points),
                       dtype=np.float32)  # разница медлу прямой и ландшафтом
    ...
    for i in range(coef * l, coef * (l + 1)):
        ...
        for j in range(count_tuples):
            for k in range(count_shifts):
                ...
                flagResExp, p3, result[ind][j][k][2], result[ind][j][k][1], rejects[ind][j][k][2], rejects[ind][j][k][
                    1] = \
                    calculatePoint3(image, p1, p2, result[ind][j][k][2],
                                    result[ind][j][k][1])  # так как последний это 3-1
...
if __name__ == '__main__':
    with Pool(count_thr) as p:
        allVal = p.map(calcMultiThreads,
                       range(count_thr))  # первый индекс кол-в потоков, второй кол-во перемен(в нашем случае3)
