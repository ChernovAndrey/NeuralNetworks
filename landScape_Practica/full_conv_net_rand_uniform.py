...
def getModel(input_shape=(32,32,2)):
    ...
    model.add(Conv2D(64,kernel_size=(7,7),padding='same',activation='relu',strides=(1,1),input_shape=input_shape,kernel_initializer=ki.RandomUniform(uniform_min_val,uniform_max_val) )) # 32*32
    model.add(Dropout(0.5))    
    model.add(MaxPooling2D(pool_size=(3,3))) #16*16
    model.add(BatchNormalization())

    model.add(Conv2D(64,kernel_size=(5,5),strides=(1,1),activation='relu',padding='same', kernel_initializer=ki.RandomUniform(uniform_min_val,uniform_max_val)  ))  #8*8
    model.add(Dropout(0.5))   
    model.add(MaxPooling2D(pool_size=(2, 2)))
    model.add(BatchNormalization())

    
    model.add(Conv2D(32,kernel_size=(3,3),padding='same',activation='relu', kernel_initializer=ki.RandomUniform(uniform_min_val,uniform_max_val) )) #4*4
